...
def refresh():
    ...
    try:
        with urllib.request.urlopen(req) as resp:
            ...
            # print(json.dumps(resp_data)) # Don't print full JSON to avoid polluting output with sensitive info if captured
    ...
    if token:
        ...
        for line in lines:
            if line.startswith('GOOGLE_ACCESS_TOKEN='):
                new_lines.append(f'GOOGLE_ACCESS_TOKEN={token}\n')
                token_found = True
            else:
        ...
        print("GOOGLE_ACCESS_TOKEN updated in .env")
