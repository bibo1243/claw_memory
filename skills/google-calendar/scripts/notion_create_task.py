...
def add_task(database_id, name, status, do_date, context, image_url=None):
    ...
    # Add image as a cover or block content? 
    # Spec said "link to Tasks", user said "Diary". 
    # Let's add it as content block (Image) inside the page.
    if image_url:
        body['children'] = [
            {
                "object": "block",
...
if __name__ == '__main__':
    if len(sys.argv) < 5:
        print("Usage: notion_create_task.py <db_id> <name> <status> <date> <context> [image_url]")
        sys.exit(1)
