...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `openclaw-skills-word-docx`                                |
| Version    | 1.0.1                                     |
| Author     | openclaw                                 |
...
**GitHub:** [openclaw/skills](https://github.com/openclaw/skills) — ⭐ 1910 | Forks: 598
...
DOCX skill reads and generates Word (.docx) files by operating on the underlying ZIP/XML structure (for example word/document.xml and word/styles.xml). It models runs (<w:r>), paragraphs (<w:p>), sections, and headers/footers so consumers avoid incorrect assumptions about text blocks, layout, and per-section pagination. The skill distinguishes reusable named styles versus inline direct formatting (including linked styles), enabling style cleanup and predictable formatting. It understands complex numbering (abstractNum, num, numId), list indentation and restart behavior, and separate numbering for bullets and numbers. It preserves and interprets revision markup and comments (<w:del>, tracked changes, commentRangeStart/end) and handles fields and cached results (TOC, page numbers, DATE, cross-references) that require updating. Typical use cases: template-driven document generation, programmatic report creation or merging, format cleanup, migration and cross-platform compatibility while retaining metadata and layout fidelity.
...
```bash
npx -y @lobehub/market-cli skills install openclaw-skills-word-docx
```
...
npx -y @lobehub/market-cli skills install openclaw-skills-word-docx --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install openclaw-skills-word-docx --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install openclaw-skills-word-docx --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install openclaw-skills-word-docx --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install openclaw-skills-word-docx --version 1.0.1
```
...
```bash
npx -y @lobehub/market-cli skills rate openclaw-skills-word-docx --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment openclaw-skills-word-docx -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comments openclaw-skills-word-docx
```
